#!/usr/bin/env python3
"""Compute bootstrap confidence intervals for MMS-Bench results."""
import json, sys, os
import numpy as np
...
def bootstrap(spec, mol, n_boot=1000, seed=42):
    rng = np.random.RandomState(seed)
    N = spec.size(0)
    accs = []
    for _ in range(n_boot):
        idx = rng.choice(N, N, replace=True)
        acc = top1_acc(spec[idx], mol[idx])
        accs.append(acc)
...
def main():
    ...
    # Also do SPECTRE dropout
    for name, path in [("full_caps_100ep", ckpt_path),]:
        print(f"\n=== {name} ===")
        ...
        ds = FastMultimodalDataset("data/processed", split="test", max_samples=10000)
        ...
        for sid, mask in MMS.items():
            ...
            mean, lo, hi = bootstrap(spec, mol, n_boot=500)
            ci = (hi - lo) / 2
        ...
        with open(f"results/bootstrap_{name}.json", "w") as f:
            json.dump(results, f, indent=2)
        print(f"Saved: results/bootstrap_{name}.json")
